...
export function useProductById(id: number, lang?: "th" | "en" | "ja") {
    ...
    const reload = useCallback(async (options?: { isInitial: boolean }) => {
        try {
            ...
            const res = await getProductById({ p_id: Number(id), lang: lang ?? "th" })
            setData(res)
        } catch (error) {
            console.log(error)
        } finally {
    ...
    }, [id, lang])  // เปลี่ยนเมื่อ id เปลี่ยนเท่านั้น
